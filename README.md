...
1. **Загрузка pfSense и запуск ранних команд.** Патч `rc.initial` запускает `ResizeZfs ` и `ContextOnly ` один раз в первые 180 секунд аптайма, избегая повторов с помощью файла `/var/run/contextallrun_started`.
...
Рекомендуемый альтернативный запуск без патча — прописать `<earlyshellcmd>/etc/context.d/ContextOnly </earlyshellcmd>` в `config.xml`, о чём напоминает заголовок `ResizeZfs`, однако текущая поставка использует именно патч `rc.initial`.
