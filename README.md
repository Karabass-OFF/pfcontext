...
Все переменные передаются через `context.sh`, который предоставляет OpenNebula. Поддерживается два семейства префиксов (`ETHx_*` и `ETHERNETx_*`) — они взаимозаменяемы. Ниже приведены основные группы.

### Сетевые параметры
| Переменная | Назначение |
| --- | --- |
| `ETHx_MAC` | MAC-адрес сетевого интерфейса, по которому сопоставляется физический порт с параметрами контекста.|
...
| `ETHx_MASK` | Маска сети. Пересчитывает её в префикс (/CIDR) и сохраняет в `config.xml` и интерфейсной конфигурации.|
...
Дополнительно, если тип не указан, автоматически назначается `lan` для приватных сетей и `wan` для публичных, заполняя остальные интерфейсы последовательными `optN`.
