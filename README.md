...
### Модуль управления интерфейсом mgmt.sh

`mgmt.sh` выполняется после основной сетевой инициализации (например, вручную либо как часть автоматизации) и переводит выбранный интерфейс в режим безопасного управления. Скрипт считывает переменные контекста `MGMT_ENABLE`, `MGMT_IF` и `MGMT_PORT` (со значениями по умолчанию `NO`, `lan` и `22,443`) и далее:

1. Определяет реальное имя интерфейса через `get_real_interface()` и ждёт его появления в системе, чтобы не применять настройки преждевременно.
2. При включении (`MGMT_ENABLE=YES`) отключает anti-lockout правило WebGUI (`noantilockout=yes`), удаляет шлюз с выбранного интерфейса, создаёт алиас `MGMT_PORTS` с перечисленными портами, а затем формирует набор правил firewall `[MGMT]`: разрешает ICMP до IP интерфейса, открывает TCP только для портов из алиаса и блокирует остальной трафик. После этого вызываются `filter_generate_aliases_config` и `filter_configure` для применения изменений.
3. При выключении (`MGMT_ENABLE!=YES`) выполняет обратные действия: возвращает anti-lockout, удаляет `[MGMT]`-правила и алиас `MGMT_PORTS`, очищает шлюз интерфейса и снова применяет конфигурацию фильтра.
4. Все шаги логируются в `/var/log/context.log` с префиксом `[context-MGMT]`, включая вывод промежуточных PHP-команд.

Таким образом, модуль позволяет держать один из интерфейсов в режиме управляемого доступа без необходимости вручную настраивать firewall после каждого запуска контекста.
